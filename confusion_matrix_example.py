import numpy as np
np.random.seed(1234) # for reproduction
...
# code used to generate confusion marix examples ()
...
def ex_c():
    data = []
    data.append( get_data( n=2500 ) )
    data.append( simplify(data[0][0], data[0][1], data[0][2], positive=1, negative=3) )
    primary = [ 1, 0 ] # subject class

    f_name = ['full', 'simplified']

    for [y_true, y_pred, labels], prim, fn in zip(data,primary,f_name):
        fig, ax = plt.subplots( figsize=( 5, 5 ), tight_layout=True )
        confusion_matrix_ax( ax, y_true, y_pred, labels, primary=prim)

        plt.savefig( 'confusion_matrix_' + fn + '.png', dpi=200 )
        #plt.show()
...
def ex_b():
    ...
    plt.tight_layout(w_pad=4)
    plt.savefig('fuzzy_matching.png', dpi=300)
    plt.show()



if __name__ =='__main__':
    if False: # used in paper draft
    ...
    # thesis
    if True:
        ex_c() # confusion matrix, multilabel
        ex_b() # confusion matrix, fuzzy accuracy
