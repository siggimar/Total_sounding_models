...
def get_data( n=1, var_1='q_n', var_2='f_dt' ):
    ...
    for fdt, qn, label in zip(X_sens_fdt, X_sens_qn, Y_sens): # y_axis
        ...
        if var_1=='q_n':
            ...
            x.append( np.average(qn) ) #np.take(qn, qn.size // 2)
            x[-1] = max(x[-1], min_F_dt) # disregard negative feed force        
        labels.append( label )
...
def data_to_kde( x, y, logx=True, logy=True, N=100, xlims=(1e-1,1e4), ylims=(1e-3,1e2) ):
    ...
    xlims = (np.log10(xlims[0]),np.log10(xlims[1]))
    ylims = (np.log10(ylims[0]),np.log10(ylims[1]))
...
def format_jointplot_ax( ax_main, ax_top, ax_side, xlims, ylims, logx, logy, rc, rc_lims, detailed ):
    ...
    # ticklabel size
    ax_main.xaxis.set_tick_params(labelsize=f_size_labelsize)
    ax_main.yaxis.set_tick_params(labelsize=f_size_labelsize)
    ...
    #if rc[0]==rc_lims[0]: ax_main.set_xlabel( '' + r'$\widebar{F_{DT}}$' + ' (kN)', fontsize=f_size_axlabels ) 
...
def jointplot_axs( 
    ...
    if data:
        all_data = list(data_to_kde( x,y, logx=logx, logy=logy, N=N, xlims=xlims, ylims=ylims ))
    ...
    if mode: ax_main.plot( x_top, y_top, marker='o', mec=(0,0,0), mew=1, mfc=c, ms=ms_mode, ls='none', label=labelm, zorder=i+20, alpha=1 )
    if data: ax_main.plot( x_, y_, marker=m, mec=(1,1,1), mew=0.6, mfc=c, ms=ms_data, ls='none', zorder=i, alpha=0.5 )
    ...
    if mode:
        ax_side.plot( yz.max(), y_top, marker='o', mec=(0,0,0), mew=1, mfc=c, ms=ms_mode, ls='none', zorder=i+20, alpha=1,clip_on=False )
        ax_top.plot( x_top, xz.max(), marker='o', mec=(0,0,0), mew=1, mfc=c, ms=ms_mode, ls='none', zorder=i+20, alpha=1,clip_on=False )
...
if __name__=='__main__':
    ...
    for i in range(1,2):
        summary(n=i, f_name='fig_8_simplified_03_SBT_chart.png', detailed=False, N=100) # 'all_classes_' + str(i) + '.png', all_classes_' + str(i) + '.png # N > 300 useable - 500 used in paper.