...
''' Script to compare functionality of CV funtions
    ...
    https://scikit-learn.org/stable/auto_examples/model_selection/plot_cv_indices.html (accessed: 2024-10)
'''
...
def cv_function_splits(CV, d_set): # used to generate thesis CV figures
    # define figure
    fig, ax = plt.subplots( figsize=(5.8, 4), tight_layout=True )
    type_color = {}

    # load data
    X, y, g, all_types = load( d_set )


    counts = np.bincount(g)
    print(counts)
    print( np.sort( np.round( counts/np.sum(counts)*100,1) ) )
    ...
    plot_cv_indices( CV, X, y, g, ax, 10, (1, 0), all_types, lw=12, suppress_y_label=True  )
    ...
    if True: # add legend
        ...
        #legend = ax.legend(c_labels, l_labels, loc='upper center', bbox_to_anchor=(0.5, -0.2), ncol=3, frameon=False, fontsize=14)

    plt.savefig('CV-' + type(CV).__name__ + '-d_set(' + str(d_set) + ').png', dpi=150, transparent=False)
    plt.show()
...
def CV_function_divergence( ds_from, ds_to ):
    score_file = 'cv_divergence.pkl'
    f_size_axlabels = 18
    f_size_labelsize = 16
    ...
    all_res = load_scores( score_file ) # load stored calculations
    if not all_res:
        for idx in range( ds_from, ds_to ):        
            ...
            save_scores(all_res, score_file) # store calculations (save time / work on presentation)

    fig, ax = plt.subplots( figsize=(10.5,5), tight_layout=True ) # (10.5,8)
    folds = np.arange( k )
    ...
    for cv in all_res:
        for measure in all_res[ cv ]:
            if measure=='CCE': continue
            
            for dataset_ver in all_res[cv][measure]:
                if dataset_ver%10==0: vals = np.array([]) # reset average array
                ax.plot( folds, all_res[cv][measure][dataset_ver], c=colors[cv], ls='--', lw=1.5, alpha=.5 )
                vals = np.append(vals, all_res[cv][measure][dataset_ver] )

                if (dataset_ver+1)%10==0: 
                    ax.plot( folds, [np.average(vals)]*len(folds), c=colors[cv], ls='-', lw=3, label= 'Average - ' + cv )
                    ax.plot( folds-10, all_res[cv][measure][dataset_ver], c=colors[cv], ls='--', lw=1.5, alpha=.5, label=cv + ' - each dataset version' ) # out of view

    
    ax.xaxis.set_tick_params(labelsize=f_size_labelsize)
    ax.yaxis.set_tick_params(labelsize=f_size_labelsize)
    ax.set_xticks(folds)
    ax.set_xlabel('Fold, ' + r'$j$' + ' (-)', fontsize=f_size_axlabels)
    ax.set_ylabel(r'$D_{KL}$' + ' (-)', fontsize=f_size_axlabels)
    ax.set_ylim((0,0.12))
    ax.set_xlim((folds[0],folds[-1]))

    handles, labels = plt.gca().get_legend_handles_labels()
    idx = np.argsort(labels).astype(int)

    plt.legend( [handles[i] for i in idx], [labels[i] for i in idx], fontsize=f_size_labelsize ) #f_size_axlabels
    plt.savefig('CV-divergence.png', dpi=100, transparent=False)
    plt.show()
...
def plot_cv_indices( cv, X, y, group, ax, n_splits, ax_id, used_types, lw=10, suppress_y_label=False ):
    ...
    if not suppress_y_label: y_title += ' - ' + r'$\Delta$' + 'D=' + info
    ...
    if ax_id[0]==0: ax.set_title('{}'.format(''+type(cv).__name__), fontsize=12)
    if ax_id[1]!=0: ax.set_yticks([])

    ax.yaxis.set_label_coords(-0.08,.6)

    return ax


def cv_function_comparison():
    datasets = [ 13, 15 ] # 2 or more

    k = 10 # desired folds
    j = -1

    fig, ax = plt.subplots( len(datasets), 2, figsize=(12, 3.5*len(datasets)), tight_layout=True )
    ...
    for idx in datasets:
        ...
        plot_cv_indices( gkfold, X, y, g, ax[j][jj], k, (j, jj), all_types, lw=10  )
        plot_cv_indices( sgkfold, X, y, g, ax[j][jj+1], k, (j, jj+1), all_types, lw=10 )
...
def mix_colors( g ):
    # defines color palette for group numbers
    global color_norm
    global c_idx
...
def sort_data( X, y, g, sort_classes=True ):
    # first sort by groups,  then optionally by class within groups
    counts = np.bincount( g )
    sorted_indices = np.argsort(counts)[::-1]

    if True: # find % of groups with less than 10% of set: 62/65=95.4%
        some_sum = np.sum(counts)
        k=0
        n=0
        for c in counts:
            if c>0:
                n+=1
                perc=round((c/some_sum)*100,1)
                print( str(perc) +'%')
                if perc<10:
                    k += 1
        print(n)
...
if __name__=='__main__':
    if False: # CV splits
        CVs= [ GroupKFold(n_splits=10), StratifiedGroupKFold(n_splits=10) ]
        cv_function_splits( CVs[0], 3 )
        # cv_function_comparison()

    if True: #Kullback-Leibler divergence
        CV_function_divergence( ds_from=0, ds_to=10 )
