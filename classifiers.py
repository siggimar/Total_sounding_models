...
class time_series_KNeighborsClassifier():
    ...
    def fit( self, X, y, set_labels=False ):
        self.X, self.y = X, y

        if True: # set list of possible labels for probabilistic 
        ...
        if not hasattr(self, 'r'): self.r = 0 # no endpoint relaxation
        self.set_endpoint_relax_r( self.r )
    ...
    def set_w( self, w=1 ):
        min_w = 1 # DTAI Distance documentation: w=1: Euclidean distance
        self.w = w
        window = max( min_w ,int(w * len(self.X[0])) ) # window to steps ( not % of length )
        print( 'window set to ' + str(window) + ' units' )
        self.dist.set_window( window ) # set dtw calculation window
    

    def set_endpoint_relax_r( self, r=0 ): # endpoint relaxation: i.e. 'r' in psi-dtw
        relax = max( 0 ,int(r * len(self.X[0])) ) # % to idx
        print( 'endpoint relax. set to ' + str(relax) + ' units' )
        self.r = relax
        self.dist.set_relaxation( relax ) # set dtw endpoint relaxation
    ...
    def calc_distances( self, X_test ): # tested {tuple(some_x):[dwt_dists]}: much better readability but way too slow!
        ...
        for i in range( len(X_test)): # for all data in series
            ...
            if dist_to_self: # utilize symmetry for the validation
                for j in range(i, len(self.X)):
                    self.distances[self.b_id][self.w][i][j] = self.dist(X_test[i], self.X[j]) # calculate distance
                    self.distances[self.b_id][self.w][j][i] = self.distances[self.b_id][self.w][i][j] # when validating with entire set
            else:
                for j in range( len(self.X) ):
                    self.distances[self.b_id][self.w][i][j] = self.dist(X_test[i], self.X[j]) # classify new point: calculate all distances
    ...
    class distance_score_dtw(): # dynamic time warping distance
        ...
        def set_relaxation( self, r ):
            self.psi = r


        def __call__( self, s_1, s_2 ):
            return dtw.distance_fast( s_1, s_2, window=self.window, psi=self.window, use_pruning=True ) # 0 to np.inf #self.psi, self.window
    ...
    class distance_score_r(): # Pearson's correlation coefficient
        ...
        def set_relaxation( self, relaxation ): print('\'endpoint relaxation, r\' argument not used in Pearson\'s r') # as with dtw variant
        def __call__( self, s_1, s_2 ):
