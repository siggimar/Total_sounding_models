...
# script used to generate plots showing validation results from SBT classifier models
...
class res_plotter():
    ...
    def reset_fig( self ):
        plt.close('all')
        self.fig, self.ax = plt.subplots( figsize=(10,5), tight_layout=True ) #(10.5,8) # #figsize=(10,4)


    def metric_settings( self, metric ):
        settings = { # store plotter settings
            'acc_0': {'label': 'Accuracy [%]', 'lims': [16, 32] },
            'acc_1': {'label':'Fuzzy accuracy' + r'$_{n=1}$' + ' [%]', 'lims': [50, 64]},
            'prec_mac': {'label':'Precision, PPV [%]', 'lims': [10, 22]},
            ...
            'rec_mac': {'label':'Recall, TPR [%]', 'lims': [8, 18]},
            ...
            'f1_mac': {'label':r'$F_1$' + ' score, ' + r'$F_1$' + ' [%]', 'lims': [0, 18]},
            ...
            'roc_auc_ovo': {'label':'ROC AUC' + r'$_{total.ovo}$' +  ' [%]', 'lims': [48, 66]},
        }
    ...
    def heatmap( self, metric='acc_0', f_name=None, k_w=None ):
        ...
        if k_w is not None:
            if metric != 'roc_auc_ovo':
                c_plt = self.ax.contour( xgrid[0], xgrid[1], ygrid, [ self.chart_scores[metric] ], linestyles=['-'], linewidths=[5], colors=[ (0,1,1) ] ) # draw countours
                m_score = str( round(self.chart_scores[metric], 1) ) + '%'
                self.ax.plot([1,150], [-50,-50], ls='-', lw=5, c=(0,1,1), label='SBT Chart: ' + m_score)
                plt.legend( fontsize=self.f_size_labelsize, framealpha=1, loc='upper right' )
...
def res_statistics():
    min_max = False # written to calibrate color scales
    max_selected_metrics = False
    max_selected_range = True



    var = ['f_dt','q_n'][1]
    w0 = [True,False][1] # this one used r=w*0
    ...
    interesting_metrics = [ 'acc_0', 'acc_1', 'prec_mac', 'rec_mac', 'f1_mac', 'roc_auc_ovo' ]
...
def plot_qn_gsa( w0=False ):
    plotter = res_plotter(n_int_rbf=200)
