...
class rate_plotter():
    ...
    def __init__( self, result_file, split_by='site', fill=True ):
       self.fill = fill
       self.data = pd.read_csv( result_file )
       self.split_by = split_by # group points by: 'site', 'unit' or 'position'
    ...
    def k_3d_plot( self ):
        ...
        (( x, v, z ), labels), var_list = self.vars_3D_figs( 'Bq__value_avg', 'rate_avg', 'qt_k' ) #'Bq__value_avg', 'rate_avg', 'qt_k' #'Bq__value_avg', 'rate_avg', 'Qt__k'
    ...
    def k_flat_plots2( self, split_val=99 ):
        ...
        # data
        (( Bq, v, k ), labels), var_list = self.vars_3D_figs('Bq__value_avg', 'rate_avg', 'fs_k') #'Bq__value_avg', 'rate_avg', 'qt_k'
    ...
    def du_flat_plot( self, split_val=0.3 ):
        ...
        for i, label in enumerate( labels ):
            ...
            if 'iller' in label:
                ...
                model_label= r'$k_{\Delta u_2}=0.07$'+ '·tanh' + r'$(\frac{v-5}{16.75})+0.95$'
    ...
    def V_2d_Nplot( self, vars, lims=None, logs=None, legend=0, letter='A' ):
        ...
        x_label = self.get_label(all_vars[11])
...
if __name__=='__main__':
    ...
    #plotter.V_2d_Nplot( vars=[16,20,28], lims=[(0,1.4e4),(0,1e2),(-4e2,1.2e3)], logs=[False,False,False], legend=2 ) #qt:16, fs:20, u2:24, du:28 :: 
    #plotter.V_2d_Nplot( vars=[48,56,52], lims=[(1e-1,1e3),(2e-2,1e1),(-5e-1,1.5e0)], logs=[True,True,False], legend=1 ) #Qt:48, Fr:56, Bq:52
    ##plotter.V_2d_Nplot( vars=[48,65,52], lims=[(1e0,1e3),(1e-2,2e0),(-8e-1,1.5e0)], logs=[True,True,False], legend=2 ) #Qt:48, fs/sigv0_eff:65, Bq:52
