...
import numpy as np
from curve_correlation.depth_warps import depth_warps
from curve_correlation.PCC_warp import PCC_warp
...
class depth_adjuster():
    ...
    def plot_all( self, warped=True, fontsize=None, tick_f_size=None, apply_drops=False, draw_units=True ):
        ...
        x_lims = { 
            ...
            2:[[0, 2000],[0,70],[0,750],[0,10]], # [[0, 3000],[0,70],[0,750],[0,10]]
        ...
        fig, ax = plt.subplots(1,4, figsize=(12,9), sharey=True, gridspec_kw={'width_ratios': [ 3, 3, 3, .8 ]} )#, sharex=True)
        ...
        for s in self.dataset.soundings:
            ...
            for i, var in enumerate( plt_vars ):
                ...
                ax[i].plot(curve_data, curve_depth, label=s.target_rate, lw=lw, c=self.dataset.rate_colors[s.target_rate], zorder=zorder)
                ax[i].tick_params( labelsize=tick_f_size ) # top=True, labeltop=True, bottom=False, labelbottom=False,

        soil_model = self.dataset.soundings[0].soil_stress_model
        ax[2].plot(soil_model.u0, soil_model.d, label='u_0', lw=0.8, ls='--',c=(0,0,0), zorder=zorder)

        handles, labels = ax[1].get_legend_handles_labels()
        by_label = dict(zip(labels, handles))

        # count occurences
        by_label = { k + ' (' + str(labels.count(k)) + 'x)': v for k, v in by_label.items() }
