...
class statm_loess(): # wrap statsmodel function implementation in a class for testing/comparison
...
def lowess_example():
    ...
    smoother = lowess( delta=0.3 )
    smoother.fit( x_1, y_n )

    y_lowess = smoother.predict()

    fig, ax = plt.subplots()
    ax.plot( x_1, y+y_rising_trend, label='base curve', zorder=2)
    ax.plot( x_1, y_n, label='noisy curve', lw=0.8, zorder=1 )
    ax.plot( x_1, y_lowess, label='noisy curve - lowess', zorder=3 )
