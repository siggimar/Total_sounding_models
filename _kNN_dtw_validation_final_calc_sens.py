...
# script to run validation calculations for dtw sensitivity screening models
# because of the calculationally intensive dtw, and the number of curves in 
# the dataset, these took a very long time!. Increasingly so with delta_d>1m
...
def validation_calc_final( res, data, ws, rs, ks, f_name, rnd_groups ):
    ...
    if True:
        ...
        d_labels[d_labels == 0] = 1 # add quick clay to sensitive index (Brittle already there)
        d_labels[d_labels == 2] = 0 # move Not sensitive to index 0
...
def calc_fold_scores( results, k_i, y_test, y_pred, y_prob ):
    ...
    results[k_i][ 'roc_auc_ovo' ].append( roc_auc_score( y_test, y_prob[:,1] ) ) # mix for sensitivity set
...
def calc_windows( some_d ):
    ...
    eps = 1e-5
    dw = max( 0.05, 1/len(some_d) ) # at least 4% between steps
