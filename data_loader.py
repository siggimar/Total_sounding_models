...
def load( set_nr=0, data_column='f_dt', scale_data=False, return_depth=False, min_leaf=25 ):
    '''
    ...
    # 'full class' also works for rko: has 100 classes (here simplified to primary- & first secondary fraction)
    label_column = 'full class' if 'prv' in some_dataset.dataset_filename else 'max two class'
    ...
    X = np.array( X )
    D = np.array( D )
    y_type = np.array( y_type, dtype='<U50' )
    g = np.array( g )
...
def prep_labels( y_type, simplify_labels=True, min_leaf=25 ):
    ...
    y_type = translate_labels( y_type=y_type ) # lowercase english classes
...
class data():
    def __init__( self, root, set_nr=0, read_json=True ):
        self.root=root
        self.set_nr = set_nr
        self.get_filename()
