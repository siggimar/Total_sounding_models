...
# script used to plot model validation results for a draft paper
# requires validation be finished and stored as picles in 'saves folder.

def acc_calc( dataset, w, k, fuzzy_matching=0 ): # faster version:  acc_calc_n() below
...
def plot_acc_calc( dataset=11, fuzzy_matching=0, f_name='saves/acc_scores.pkl' ):
    ...
    for (ds, fm, k, w ), val in res.items():
        if ds==dataset and fm==fuzzy_matching  and w<0.99 and (k in np.arange(1,301,10)) and np.abs(w-0.4)>0.02: #and any(np.abs(np.arange(0,1.01, 0.05) - w) < eps)
            ks.append( k )
            ws.append( w )
            acc.append( val )
    ...
    plot_surface(ax, ks, ws, acc, cont)
...
def plot_surface( ax, ks, ws ,acc, cont=None, c_label=None ):
    ...
    if c_label is None: c_label='Accuracy (%)'
...
if __name__=='__main__':
    w = np.array( [0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.30, 0.4, 0.5, 0.6, 0.7, 0.8, 0.95,1] )
    w = np.array( [0.3] )
    w = np.arange( 0.15, 0.46, 0.05 )
    #w = np.array( [0.95] )
    #w = np.arange( 0, 1.05, 0.1 )

    k=np.arange(1, 301, 2)
    if True: # GSA classifiers
        #compare_GSA_classifiers( fuzzy_matching=0, dataset=15 )
        for i in [ 0 ]:
            pass # np.arange(0.6,0.61,0.3)            
            #acc_calc_n( dataset=15, w=w, k=k, fuzzy_matching=i, f_name='saves/acc_rnd_groups.pkl' )
        plot_acc_calc( dataset=15, fuzzy_matching=0, f_name='saves/acc_rnd_groups.pkl' )

    else: # Sensitivity classifiers
        optimal_threshold( dataset=5 ) # plots accuracy of simplified model (P>t) as a function of t.

        if False:
            plot_acc_calc( dataset=3 )
            for i in [3]:
                f_name='saves/' + str(i) + '_n0_acc_scores.pkl'
                acc_calc_sens_n( dataset=i, w=w, k=k, f_name=f_name )
