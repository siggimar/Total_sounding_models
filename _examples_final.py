...
# Script to showcase the dtw classifier on total sounding profile.
...
def classify( some_tot ): # loads classifiers, extracts desired data and returns classifications
    ...
    # load & fit GSA dtw classifiers
    qn03, y03, _, all_types = load( 11, data_column='q_n' ) # delta_D=0.3m #GSA
    dtw_clf03 = tot_dtw_clf( n_neighbors=k, w=0.25 )
    dtw_clf03.fit( qn03, y03 )

    qn05, y05, _, all_types = load( 13, data_column='q_n' ) # delta_D=0.5m #GSA
    dtw_clf05 = tot_dtw_clf( n_neighbors=k, w=0.25 )
    dtw_clf05.fit( qn05, y05 )

    qn10, y10, _, all_types = load( 15, data_column='q_n' ) # delta_D=1.0m #GSA
    dtw_clf10 = tot_dtw_clf( n_neighbors=k, w=0.25 )
    dtw_clf10.fit( qn10, y10 )

    qn20, y20, _, all_types = load( 19, data_column='q_n' ) # delta_D=2.0m #GSA
    dtw_clf20 = tot_dtw_clf( n_neighbors=k, w=0.25 )
    dtw_clf20.fit( qn20, y20 )

    qnr, yr, _, all_types = load( 20, data_column='q_n', min_leaf=10 ) # delta_D=1.0m #GSA reduced
    dtw_clfr = tot_dtw_clf( n_neighbors=kr, w=0.2 )
    dtw_clfr.fit( qnr, yr )
    ...
    # get sounding data    
    dtw03_data, dtw05_data, dtw10_data, dtw20_data, sbt_data = get_clf_data( some_tot )

    # predict dtw GSA dataset
    y_pred_dtw03 = dtw_clf03.predict( dtw03_data[0] )
    y_pred_dtw_colors03 = [ colors[all_types[some_y]] for some_y in y_pred_dtw03 ]

    y_pred_dtw05 = dtw_clf05.predict( dtw05_data[0] )
    y_pred_dtw_colors05 = [ colors[all_types[some_y]] for some_y in y_pred_dtw05 ]

    y_pred_dtw10 = dtw_clf10.predict( dtw10_data[0] )
    y_pred_dtw_colors10 = [ colors[all_types[some_y]] for some_y in y_pred_dtw10 ]

    y_pred_dtw20 = dtw_clf10.predict( dtw20_data[0] )
    y_pred_dtw_colors20 = [ colors[all_types[some_y]] for some_y in y_pred_dtw20 ]

    y_pred_dtwr = dtw_clfr.predict( dtw10_data[0] )
    y_pred_dtw_colorsr = [ colors[all_types[some_y]] for some_y in y_pred_dtwr ]

    # predict chart
    y_pred_sbt = chart_clf.predict( sbt_data[0], sbt_data[1] )
    y_pred_sbt_colors = chart_clf.class_colors( y_pred_sbt )


    res = {
         'DTW\n0.3m'   :{ 'y_pred': y_pred_dtw03, 'd': dtw03_data[1], 'colors': y_pred_dtw_colors03 },
         'DTW\n0.5m'   :{ 'y_pred': y_pred_dtw05, 'd': dtw05_data[1], 'colors': y_pred_dtw_colors05 },
         'DTW\n1.0m'   :{ 'y_pred': y_pred_dtw10, 'd': dtw10_data[1], 'colors': y_pred_dtw_colors10 },
         'DTW\n2.0m'   :{ 'y_pred': y_pred_dtw20, 'd': dtw20_data[1], 'colors': y_pred_dtw_colors20 },
         'DTWr\n1.0m'  :{ 'y_pred': y_pred_dtwr, 'd': dtw10_data[1], 'colors': y_pred_dtw_colorsr },
         'Chart\n0.3m' :{ 'y_pred': y_pred_sbt, 'd': sbt_data[2], 'colors': y_pred_sbt_colors },         
    }
...
def get_clf_data( some_tot ):
    ...
    qn_20_dm, D_20_dm = get_data( some_tot, 'q_n', 2.0 )
    ...
    return dtw03_data, dtw05_data, dtw10_data, dtw20_data, sbt_data
...
def add_clf_to_figure( plotter, clf_res, some_tot ):
    ...
    for c in clf_res:
        ...
        if 'y_pred' in clf_res[c]:
            ...
            c_axs[-1].plot([50]*2, y_lims[-1], c=(0.9,0,0), ls='--', label='t=50%')
            c_axs[-1].legend()
...
def example_profiles( n=None, save_restore=False ):
    for i, some_file in enumerate( file_list(n) ):
        ...
        some_tot, plotter = tot_data_and_figure( some_file ) # can't save plotter
        ...
        save_example( clf_res, saved_ex_name ) # always keep last calculation

        add_clf_to_figure( plotter, clf_res, some_tot)

        plt.show()
        #plt.savefig( 'ex_'+ str(n[i]) +'.png',dpi=150, transparent=False )
        a=1


if __name__=='__main__':    
    example_profiles( n=[27,21], save_restore=False ) #31,39,52,92,7,13,17,21,23], save_restore=True )